...
void MainComponent::buildSoundPanel()
{
    ...
    soundCombo = &list.addCombo ("Sound", { "Pink noise", "Pink noise pulses", "Tone 440 Hz", "Bass tone 60 Hz", "Audio file" }, 0,
                                 [this] (int i)
                                 {
                                     if ((AudioEngine::TestSound) i == AudioEngine::TestSound::audioFile && ! engine.hasAudioFile())
                                         chooseAudioFile();
                                     engine.setTestSound ((AudioEngine::TestSound) i);
                                 });
    engine.setTestSound (AudioEngine::TestSound::pinkNoise);
    list.addButtonRow ({ { "Choose audio file...", [this] { chooseAudioFile(); } } }, 26);
    fileLabel = &list.addNote ("Any WAV, AIFF, MP3 or FLAC. It loops, mixed to mono.", 20);

    list.addHeading ("Movement");
    list.addSlider ("Spread", 0.0, 4.0, 0.01, settings.dbap.blur, " m", [this] (double v)
                    { settings.dbap.blur = (float) v; pushSettings(); }, 1.0);
    list.addSlider ("Low spread", 0.0, 6.0, 0.01, settings.lowSpread, " m", [this] (double v)
                    { settings.lowSpread = (float) v; pushSettings(); }, 1.5);
    list.addSlider ("Focus", 2.0, 18.0, 0.5, settings.dbap.rolloffDb, " dB", [this] (double v)
                    { settings.dbap.rolloffDb = (float) v; pushSettings(); });
    list.addSlider ("Glide", 0.0, 2000.0, 1.0, settings.glideMs, " ms", [this] (double v)
                    { settings.glideMs = (float) v; pushSettings(); }, 200.0);
    list.addNote ("Spread: how many neighbouring speakers share the sound. More spread = softer, "
                  "smoother movement. Low spread: extra sharing for frequencies below 300 Hz, so bass "
                  "flows wider than the highs. Focus: how strongly the nearest speakers take over. "
                  "Glide: eases sudden jumps in position.", 94);
    ...
    list.addSlider ("Sub crossover", 30.0, 200.0, 1.0, settings.subCrossoverHz, " Hz", [this] (double v)
                    { settings.subCrossoverHz = (float) v; pushSettings(); }, 80.0);
    ...
    list.addNote ("Subs get everything below the crossover (80 Hz by default). Transducers get everything "
                  "below their range and follow the sound between them, so vibration moves with it.", 50);
}
...
{
    fileChooser = std::make_unique<juce::FileChooser> ("Choose a sound to move around the room",
                              ...
                              [this] (const juce::FileChooser& fc)
    {
        ...
        if (reader == nullptr)
        {
            juce::AlertWindow::showMessageBoxAsync (juce::MessageBoxIconType::WarningIcon, "Couldn't open that file",
                                                    "Try a WAV or AIFF file.");
            return;
        }

        // Up to 10 minutes, mixed down to mono
        const auto maxSamples = (juce::int64) (reader->sampleRate * 600.0);
        const int length = (int) juce::jmin (reader->lengthInSamples, maxSamples);
        const int channels = (int) juce::jmax (1u, reader->numChannels);

        juce::AudioBuffer<float> multi (channels, length);
        reader->read (&multi, 0, length, 0, true, true);

        auto mono = std::make_unique<juce::AudioBuffer<float>> (1, length);
        mono->clear();
        for (int ch = 0; ch < channels; ++ch)
            mono->addFrom (0, 0, multi, ch, 0, length, 1.0f / (float) channels);

        engine.setAudioFile (std::move (mono), reader->sampleRate);
        engine.setTestSound (AudioEngine::TestSound::audioFile);
        soundCombo->setSelectedItemIndex ((int) AudioEngine::TestSound::audioFile, juce::dontSendNotification);
        fileLabel->setText ("Playing: " + file.getFileName(), juce::dontSendNotification);
    });
}
