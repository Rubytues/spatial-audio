...
RoomView3D::RoomView3D (AudioEngine& e) : engine (e)
{
    ...
    addAndMakeVisible (percentButton);
    percentButton.setTooltip ("Show how much of the sound each speaker is playing");
    percentButton.setClickingTogglesState (true);
    percentButton.setToggleState (true, juce::dontSendNotification);
    percentButton.setColour (juce::TextButton::buttonColourId, colours::panel.withAlpha (0.85f));
    percentButton.setColour (juce::TextButton::buttonOnColourId, colours::regular.darker (0.5f));
    percentButton.setColour (juce::TextButton::textColourOffId, colours::text);
    percentButton.setColour (juce::TextButton::textColourOnId, juce::Colours::white);
    percentButton.onClick = [this] { showPercentages = percentButton.getToggleState(); repaint(); };
...
void RoomView3D::resized()
{
    ...
    percentButton.setBounds (area.removeFromRight (36));
}
...
void RoomView3D::paint (juce::Graphics& g)
{
    ...
    for (const auto& item : items)
    {
        ...
        // Share of the sound this speaker is playing, as a percentage
        const bool showShare = showPercentages && engine.isPlaying() && ! engine.isSpeakerTesting();
        if (showShare)
        {
            const int percent = juce::roundToInt (level * level * 100.0f);
            if (percent >= 1)
            {
                g.setFont (juce::FontOptions (11.0f, juce::Font::bold));
                g.setColour (colour.brighter (0.4f));
                g.drawText (juce::String (percent) + "%",
                            juce::Rectangle<float> (60.0f, 14.0f).withCentre (p.point.translated (0.0f, -r * 1.3f - 9.0f)),
                            juce::Justification::centred);
            }
        }
